package it.ac.cargoflow.view.fasciaorariacliente;
...
import it.ac.cargoflow.entity.FasciaOrariaCliente;
import it.ac.cargoflow.view.main.MainView;
import org.springframework.beans.factory.annotation.Autowired;

import static io.jmix.flowui.component.delegate.AbstractFieldDelegate.PROPERTY_INVALID;

@Route(value = "fascia-oraria-clientes", layout = MainView.class)
@ViewController(id = "FasciaOrariaCliente.list")
@ViewDescriptor(path = "fascia-oraria-cliente-list-view.xml")
@LookupComponent("fasciaOrariaClientesDataGrid")
@DialogMode(width = "64em")
public class FasciaOrariaClienteListView extends StandardListView<FasciaOrariaCliente> {

    @ViewComponent
    private DataContext dataContext;

    @ViewComponent
    private CollectionContainer<FasciaOrariaCliente> fasciaOrariaClientesDc;

    @ViewComponent
    private InstanceContainer<FasciaOrariaCliente> fasciaOrariaClienteDc;

    @ViewComponent
    private InstanceLoader<FasciaOrariaCliente> fasciaOrariaClienteDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<FasciaOrariaCliente> fasciaOrariaClientesDataGrid;
    ...
    public void onInit(final InitEvent event) {
        fasciaOrariaClientesDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("fasciaOrariaClientesDataGrid.createAction")
    public void onFasciaOrariaClientesDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        FasciaOrariaCliente entity = dataContext.create(FasciaOrariaCliente.class);
        fasciaOrariaClienteDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("fasciaOrariaClientesDataGrid.editAction")
    public void onFasciaOrariaClientesDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "fasciaOrariaClientesDc", target = Target.DATA_CONTAINER)
    public void onFasciaOrariaClientesDcItemChange(final InstanceContainer.ItemChangeEvent<FasciaOrariaCliente> event) {
        prepareFormForValidation();

        FasciaOrariaCliente entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            fasciaOrariaClienteDl.setEntityId(EntityValues.getId(entity));
            fasciaOrariaClienteDl.load();
        } else {
            fasciaOrariaClienteDl.setEntityId(null);
            fasciaOrariaClienteDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        FasciaOrariaCliente item = fasciaOrariaClienteDc.getItem();
        ...
        dataContext.save();
        fasciaOrariaClientesDc.replaceItem(item);
        updateControls(false);
    ...
    private ValidationErrors validateView(FasciaOrariaCliente entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        fasciaOrariaClientesDataGrid.getActions().forEach(Action::refreshState);
