package it.ac.cargoflow.view.contrassegnotipo;
...
import it.ac.cargoflow.entity.ContrassegnoTipo;
import it.ac.cargoflow.view.main.MainView;
import org.springframework.beans.factory.annotation.Autowired;

import static io.jmix.flowui.component.delegate.AbstractFieldDelegate.PROPERTY_INVALID;

@Route(value = "contrassegno-tipoes", layout = MainView.class)
@ViewController(id = "ContrassegnoTipo.list")
@ViewDescriptor(path = "contrassegno-tipo-list-view.xml")
@LookupComponent("contrassegnoTipoesDataGrid")
@DialogMode(width = "64em")
public class ContrassegnoTipoListView extends StandardListView<ContrassegnoTipo> {

    @ViewComponent
    private DataContext dataContext;

    @ViewComponent
    private CollectionContainer<ContrassegnoTipo> contrassegnoTipoesDc;

    @ViewComponent
    private InstanceContainer<ContrassegnoTipo> contrassegnoTipoDc;

    @ViewComponent
    private InstanceLoader<ContrassegnoTipo> contrassegnoTipoDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<ContrassegnoTipo> contrassegnoTipoesDataGrid;
    ...
    public void onInit(final InitEvent event) {
        contrassegnoTipoesDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("contrassegnoTipoesDataGrid.createAction")
    public void onContrassegnoTipoesDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        ContrassegnoTipo entity = dataContext.create(ContrassegnoTipo.class);
        contrassegnoTipoDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("contrassegnoTipoesDataGrid.editAction")
    public void onContrassegnoTipoesDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "contrassegnoTipoesDc", target = Target.DATA_CONTAINER)
    public void onContrassegnoTipoesDcItemChange(final InstanceContainer.ItemChangeEvent<ContrassegnoTipo> event) {
        prepareFormForValidation();

        ContrassegnoTipo entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            contrassegnoTipoDl.setEntityId(EntityValues.getId(entity));
            contrassegnoTipoDl.load();
        } else {
            contrassegnoTipoDl.setEntityId(null);
            contrassegnoTipoDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        ContrassegnoTipo item = contrassegnoTipoDc.getItem();
        ...
        dataContext.save();
        contrassegnoTipoesDc.replaceItem(item);
        updateControls(false);
    ...
    private ValidationErrors validateView(ContrassegnoTipo entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        contrassegnoTipoesDataGrid.getActions().forEach(Action::refreshState);
