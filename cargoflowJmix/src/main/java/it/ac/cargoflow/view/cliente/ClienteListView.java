...
public class ClienteListView extends StandardListView<Cliente> {
    ...
    @ViewComponent
    private CollectionContainer<Cliente> clientesDc;

    @ViewComponent
    private InstanceContainer<Cliente> clienteDc;

    @ViewComponent
    private InstanceLoader<Cliente> clienteDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Cliente> clientesDataGrid;
    ...
    public void onInit(final InitEvent event) {
        clientesDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("clientesDataGrid.createAction")
    public void onClientesDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Cliente entity = dataContext.create(Cliente.class);
        clienteDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("clientesDataGrid.editAction")
    public void onClientesDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "clientesDc", target = Target.DATA_CONTAINER)
    public void onClientesDcItemChange(final InstanceContainer.ItemChangeEvent<Cliente> event) {
        prepareFormForValidation();

        Cliente entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            clienteDl.setEntityId(EntityValues.getId(entity));
            clienteDl.load();
        } else {
            clienteDl.setEntityId(null);
            clienteDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Cliente item = clienteDc.getItem();
    ...
    private ValidationErrors validateView(Cliente entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        clientesDataGrid.getActions().forEach(Action::refreshState);
