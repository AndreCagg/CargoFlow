...
public class AziendaListView extends StandardListView<Azienda> {
    ...
    @ViewComponent
    private InstanceContainer<Azienda> aziendaDc;

    @ViewComponent
    private InstanceLoader<Azienda> aziendaDl;

    @ViewComponent
    private VerticalLayout listLayout;

    @ViewComponent
    private DataGrid<Azienda> aziendasDataGrid;
    ...
    public void onInit(final InitEvent event) {
        aziendasDataGrid.getActions().forEach(action -> {
    ...
    @Subscribe("aziendasDataGrid.createAction")
    public void onAziendasDataGridCreateAction(final ActionPerformedEvent event) {
        prepareFormForValidation();

        dataContext.clear();
        Azienda entity = dataContext.create(Azienda.class);
        aziendaDc.setItem(entity);
        updateControls(true);
    }

    @Subscribe("aziendasDataGrid.editAction")
    public void onAziendasDataGridEditAction(final ActionPerformedEvent event) {
        updateControls(true);
    }
    ...
    @Subscribe(id = "aziendasDc", target = Target.DATA_CONTAINER)
    public void onAziendasDcItemChange(final InstanceContainer.ItemChangeEvent<Azienda> event) {
        prepareFormForValidation();

        Azienda entity = event.getItem();
        dataContext.clear();
        if (entity != null) {
            aziendaDl.setEntityId(EntityValues.getId(entity));
            aziendaDl.load();
        } else {
            aziendaDl.setEntityId(null);
            aziendaDc.setItem(null);
        }
        updateControls(false);
    }
    ...
    private OperationResult saveEditedEntity() {
        Azienda item = aziendaDc.getItem();
    ...
    private ValidationErrors validateView(Azienda entity) {
    ...
    private void updateControls(boolean editing) {
        ...
        aziendasDataGrid.getActions().forEach(Action::refreshState);
